...
function serializeWebAuthnCredential(
  credentialRequest: PublicKeyCredentialCreationOptions
  ...
  const serialized: PublicKeyCredentialCreationOptionsJSON = {
    ...
    pubKeyCredParams: credentialRequest.pubKeyCredParams.map(
      (param: { type: any; alg: any }) => ({
        type: param.type,
        alg: param.alg,
      })
    ),
...
function serializeCredentialRequestOptions(
  ...
  if (publicKey.allowCredentials) {
    serializedPublicKey.allowCredentials = publicKey.allowCredentials.map(
      (cred) => ({
...
function parseWebAuthnCredentialGet(credentialJson: any): PublicKeyCredential {
  ...
  const userHandle = credentialJson.response.userHandle
  ...
  return {
    ...
    getClientExtensionResults:
      function (): AuthenticationExtensionsClientOutputs {
        return {};
      },
    toJSON: function () {
...
function parseWebAuthnCredentialCreate(
  credentialJson: any
): PublicKeyCredential {
  const rawId = base64UrlToBuffer(credentialJson.rawId);
  const type = credentialJson.type;
  const id = credentialJson.id;
  const clientData = base64UrlToBuffer(credentialJson.response.clientDataJSON);
  const attestationObject = base64UrlToBuffer(
...
export async function createCredential(
  cred: PublicKeyCredentialCreationOptions
) {
  const webSafeRequest = serializeWebAuthnCredential(cred);

  return await invoke<{ pubKeyJson?: string }>(
    "plugin:p256-signer|create_credential",
    {
      payload: {
...
export async function getCredential(
  getParams: CredentialRequestOptions
): Promise<PublicKeyCredential> {
  const serializedParams = serializeCredentialRequestOptions(
    getParams.publicKey!
  );

  return await invoke<{ pubKeyJson?: string }>(
    "plugin:p256-signer|get_credential",
    {
      payload: {
